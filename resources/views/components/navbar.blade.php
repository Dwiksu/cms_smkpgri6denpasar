<div id="header-wrapper">
    <nav class="bg-transparent backdrop-blur-sm sticky w-full z-50 top-0 start-0 shadow-sm" id="header"
        x-data="{ mobileOpen: false }" x-init="$watch('mobileOpen', value => {
            document.body.classList.toggle('lock-scroll', value);
        })">
        <div class="max-w-screen-xl flex flex-wrap items-center mx-auto p-4">

            {{-- Logo --}}
            <a href="{{ route('public.home.index') }}"
                class="flex items-center me-auto space-x-3 rtl:space-x-reverse z-50">
            ...
            <div class="flex items-center ml-8 gap-3 md:order-2 z-50">
                ...
                <button @click="mobileOpen = !mobileOpen" type="button"
                    class="inline-flex items-center p-2 w-10 h-10 justify-center text-sky-600 md:hidden focus:outline-none transition-colors rounded-lg"
            ...
            <div x-show="mobileOpen" x-transition:enter="transition ease-out duration-300"
                ...
                class="fixed inset-0 w-full h-[100dvh] bg-white z-40 md:hidden">

                <div class="flex flex-col h-full">
                    <div class="h-20 flex-shrink-0"></div>

                    <div class="flex-1 overflow-y-auto px-6 py-4 pb-24">
                        <p class="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] mb-4 ml-2">Menu Sekolah
                        </p>
                        ...
                        <ul class="flex flex-col space-y-2 text-sm font-semibold">
                            ...
                            <li><a href="{{ route('public.galeri.index') }}"
                                    class="block px-4 py-3.5 rounded-2xl {{ isActiveHamburgerMenu('public.galeri.index') }}">Galeri</a>
                            </li>
                            <li><a href="{{ route('public.guru.index') }}"
                                    class="block px-4 py-3.5 rounded-2xl {{ isActiveHamburgerMenu('public.guru.index') }}">Profil
                                    Guru</a></li>
                            <li><a href="{{ route('public.kalender.index') }}"
                                    class="block px-4 py-3.5 rounded-2xl {{ isActiveHamburgerMenu('public.kalender.index') }}">Kalender</a>
                            </li>
                        </ul>
                            ...
                            <div
                                ...
                                <ul class="text-sm text-body font-small divide-y divide-default/60">
                                    <li class="flex items-center w-full px-4 py-3">
                                        <a href="{{ route('public.tentang.sambutan') }}"
                                            class="{{ isActiveNavbar('public.tentang.sambutan') }} w-full">Sambutan
                                            Kepala
                                            Sekolah</a>
                                    </li>
                                    <li class="flex items-center w-full px-4 py-3">
                                    ...
                                    <li class="flex items-center w-full px-4 py-3">
                                        <a href="{{ route('public.tentang.kontak') }}"
                                            class="{{ isActiveNavbar('public.tentang.kontak') }} w-full">Kontak
                                            Kami</a>
                                    </li>
                                </ul>
                            </div>
...
<script>
    ...
    function adjustHeader() {
        ...
        if (scrollPos > 0) {
            // Saat scroll: Jadi Fixed & Beri Background (biar teks terbaca)
            header.classList.add('fixed');
            header.classList.remove('sticky');

            // Wrapper menahan ruang agar konten di bawah TIDAK terpotong
            wrapper.style.height = headerHeight + 'px';
        } else {
            // Saat di paling atas: Kembali Normal
    ...
    window.addEventListener('resize', adjustHeader); // Jaga-jaga kalau ganti orientasi HP
</script>
